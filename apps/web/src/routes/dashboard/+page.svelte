<script lang="ts">
   ...
   *
   * Navy hero band (untouched), then bold, tightly-scaled working surfaces:
   * a six-month income-vs-spend trend, a "cash leaving the account" forecast
   * with inline Total/Recurring/Variable boxes paired with upcoming
   * commitments (from recurring detection), a recent-ledger + ranked
   * spend-shape row, and a budgets + insights row. Real API data; the trend
...
<div class="relative">
  ...
  <!-- ── Forecast (cash leaving) + upcoming commitments ────────────── -->
