<script lang="ts">
  /**
   * Dashboard — "The Monthly Dossier".
   *
   * Not a card grid. The month reads as a long-form financial feature: a
   * headline lead (net worth as the story), then numbered editorial sections
   * — The Outlook, Money in Motion, The Ledger, Discipline — each with a
   * standfirst and an asymmetric two-column body. A sticky contents index in
   * the left margin scroll-spies the section in view and lets the reader jump.
   * Real API data throughout; the trend is six cached monthly summaries.
   */
  ...
  // Upcoming commitments — recurring items with a next date, soonest first.
  ...
  // ── Dossier contents — sections the reader can jump between, with a
  //    scroll-spy marking the one currently in view. ─────────────────
  const contents = [
    { id: 'outlook', no: 'I', label: 'The Outlook' },
    { id: 'movement', no: 'II', label: 'Money in Motion' },
    { id: 'ledger', no: 'III', label: 'The Ledger' },
    { id: 'discipline', no: 'IV', label: 'Discipline' },
  ];
  let activeSection = $state('outlook');
...
<div class="mx-auto grid max-w-[1180px] grid-cols-1 gap-x-12 gap-y-10 lg:grid-cols-[13rem_minmax(0,1fr)]">
  <!-- ══════════ Contents index — sticky scroll-spy in the margin ══════════ -->
  <aside class="hidden lg:block">
    <div class="sticky top-[10.5rem]">
      <p class="text-[10px] font-semibold uppercase tracking-[0.24em] text-[hsl(var(--muted-foreground))]">In this issue</p>
      <div class="mt-3 h-px bg-[hsl(var(--brand-navy)/0.2)]"></div>
      <nav class="mt-3 flex flex-col gap-0.5" aria-label="Dossier contents">
        {#each contents as c (c.id)}
          {@const on = activeSection === c.id}
          <button
            type="button"
            onclick={() => jumpTo(c.id)}
            class="group flex items-baseline gap-3 py-2 text-left transition-colors"
            aria-current={on ? 'true' : undefined}
          >
            <span class={['font-display text-xs tabular-nums transition-colors', on ? 'text-[hsl(var(--brand-gold))]' : 'text-[hsl(var(--muted-foreground)/0.6)]'].join(' ')}>{c.no}</span>
            <span class="relative">
              <span class={['text-sm transition-colors', on ? 'font-semibold text-[hsl(var(--brand-navy))]' : 'font-medium text-[hsl(var(--muted-foreground))] group-hover:text-[hsl(var(--brand-navy))]'].join(' ')}>{c.label}</span>
              <span class={['absolute -bottom-0.5 left-0 h-px bg-[hsl(var(--brand-gold))] transition-all duration-300', on ? 'w-full' : 'w-0 group-hover:w-1/2'].join(' ')}></span>
            </span>
          </button>
        {/each}
      </nav>

      <!-- A small standing "ask" call, like a margin note -->
      <div class="mt-6 border-t border-[hsl(var(--border))] pt-5">
        <p class="text-xs leading-relaxed text-[hsl(var(--muted-foreground))]">Have a question about these figures?</p>
        <button type="button" onclick={() => ask(m.dashboard.promptWhereDidMyMoneyGo)} class="group mt-2 inline-flex items-center gap-1.5 text-sm font-semibold text-[hsl(var(--brand-navy))]">
          <Sparkles class="h-3.5 w-3.5 text-[hsl(var(--brand-gold))]" /> {m.nav.askCopilot}
  ...
  <article class="flex min-w-0 flex-col">
    <!-- ── Lead: net worth as the headline story ───────────────────── -->
    <header class="relative">
      ...
      <div class="grid grid-cols-1 gap-6 pt-6 lg:grid-cols-[1.9fr_1fr] lg:gap-10">
        ...
        <div class="flex flex-col items-center justify-center gap-4 lg:border-l lg:border-[hsl(var(--border))] lg:pl-10">
          ...
          <p class="text-center text-xs leading-relaxed text-[hsl(var(--muted-foreground))]">of income kept this month</p>
        </div>
      </div>
    </header>

    <!-- ════ I · The Outlook — forecast + commitments ════ -->
    <section id="outlook" class="scroll-mt-[10rem] pt-12">
      {@render sectionHead('I', 'The Outlook', 'What the next thirty days hold')}
      <div class="mt-6 grid grid-cols-1 gap-x-10 gap-y-8 lg:grid-cols-[1.55fr_1fr]">
        <div>
          <div class="flex flex-wrap items-end justify-between gap-3">
            <div>
              <p class="text-[11px] font-medium uppercase tracking-[0.16em] text-[hsl(var(--muted-foreground))]">Cash leaving the account</p>
              {#if forecast.data?.forecast}
                <p class="mt-1 font-display text-3xl font-semibold tabular-nums text-[hsl(var(--brand-navy))]">{formatCurrency(Math.round(forecast.data.forecast.total))}</p>
              {/if}
            </div>
            {#if forecast.data?.forecast}
              <div class="flex gap-5 text-right">
                <div>
                  <p class="text-[10px] uppercase tracking-[0.12em] text-[hsl(var(--muted-foreground))]">{m.forecast.recurring}</p>
                  <p class="font-display text-base font-semibold tabular-nums">{formatCurrency(Math.round(forecast.data.forecast.recurringBase))}</p>
                </div>
                <div>
                  <p class="text-[10px] uppercase tracking-[0.12em] text-[hsl(var(--muted-foreground))]">{m.forecast.variable}</p>
                  <p class="font-display text-base font-semibold tabular-nums">{formatCurrency(Math.round(forecast.data.forecast.variableTotal))}</p>
                </div>
              </div>
            {/if}
          </div>
          <div class="mt-4">
            {#if forecast.loading}
              <div class="h-[160px] w-full animate-pulse rounded-lg bg-[hsl(var(--muted))]"></div>
            {:else if forecast.data?.forecast.daily?.length}
              <ForecastStrip daily={forecast.data.forecast.daily} total={forecast.data.forecast.total} height={160} />
            {:else}
        ...
        <!-- Commitments, set as a margin sidebar -->
        <div class="lg:border-l lg:border-[hsl(var(--border))] lg:pl-10">
          <p class="text-[11px] font-medium uppercase tracking-[0.16em] text-[hsl(var(--muted-foreground))]">Commitments ahead</p>
          <div class="mt-3">
            ...
            {:else if upcoming.length > 0}
              ...
              <div class="mt-3 flex items-center justify-between border-t border-[hsl(var(--brand-navy)/0.2)] pt-3 text-sm">
                <span class="text-[hsl(var(--muted-foreground))]">Next ~30 days</span>
                <span class="font-display font-semibold tabular-nums text-[hsl(var(--brand-navy))]">{formatCurrency(Math.round(upcomingTotal))}</span>
              </div>
            {:else}
              <div class="py-8 text-center">
                <Wallet2 class="mx-auto mb-2 h-5 w-5 text-[hsl(var(--muted-foreground))]" />
    ...
    <!-- ════ II · Money in Motion — six-month trend ════ -->
    <section id="movement" class="mt-12 scroll-mt-[10rem] border-t border-[hsl(var(--brand-navy)/0.12)] pt-12">
      <div class="flex flex-wrap items-end justify-between gap-3">
        {@render sectionHead('II', 'Money in Motion', 'Six months of income against spend')}
      ...
      <div class="mt-6">
        {#if trendLoading}
          <div class="h-[230px] w-full animate-pulse rounded-lg bg-[hsl(var(--muted))]"></div>
        {:else if trend.some((t) => t.income > 0 || t.expense > 0)}
          <TrendChart months={trend} height={230} />
        {:else}
          <div class="grid h-[230px] place-items-center text-center text-sm text-[hsl(var(--muted-foreground))]">{m.dashboard.emptyTransactions}</div>
        {/if}
      </div>
    </section>

    <!-- ════ III · The Ledger — recent + spend shape ════ -->
    <section id="ledger" class="mt-12 scroll-mt-[10rem] border-t border-[hsl(var(--brand-navy)/0.12)] pt-12">
      {@render sectionHead('III', 'The Ledger', 'Lately, and where it went')}
      <div class="mt-6 grid grid-cols-1 gap-x-10 gap-y-8 lg:grid-cols-2">
        <div>
          <div class="flex items-center justify-between">
            <p class="text-[11px] font-medium uppercase tracking-[0.16em] text-[hsl(var(--muted-foreground))]">{m.dashboard.recent}</p>
            <a href="/transactions" class="inline-flex items-center gap-1 text-xs font-medium text-[hsl(var(--brand-navy))] hover:underline">All <ArrowRight class="h-3 w-3" /></a>
          </div>
          <div class="mt-3">
        ...
        <div class="lg:border-l lg:border-[hsl(var(--border))] lg:pl-10">
          <div class="flex items-center justify-between">
            <p class="text-[11px] font-medium uppercase tracking-[0.16em] text-[hsl(var(--muted-foreground))]">{m.dashboard.topCategories}</p>
            <a href="/reports" class="inline-flex items-center gap-1 text-xs font-medium text-[hsl(var(--brand-navy))] hover:underline">Reports <ArrowRight class="h-3 w-3" /></a>
          </div>
          <div class="mt-4">
            {#if summary.loading}
    ...
    <!-- ════ IV · Discipline — budgets + insights ════ -->
    <section id="discipline" class="mt-12 scroll-mt-[10rem] border-t border-[hsl(var(--brand-navy)/0.12)] pt-12">
      {@render sectionHead('IV', 'Discipline', 'Budgets held, and a word of advice')}
      <div class="mt-6 grid grid-cols-1 gap-x-10 gap-y-8 lg:grid-cols-2">
        <div>
          <div class="flex items-center justify-between">
            <p class="text-[11px] font-medium uppercase tracking-[0.16em] text-[hsl(var(--muted-foreground))]">{m.dashboard.budgetAlerts}</p>
            <a href="/budgets" class="inline-flex items-center gap-1 text-xs font-medium text-[hsl(var(--brand-navy))] hover:underline">All <ArrowRight class="h-3 w-3" /></a>
          </div>
          <div class="mt-4">
        ...
        <div class="lg:border-l lg:border-[hsl(var(--border))] lg:pl-10">
          <div class="flex items-center gap-2">
            ...
            <p class="text-[11px] font-medium uppercase tracking-[0.16em] text-[hsl(var(--muted-foreground))]">For you</p>
          </div>
          <div class="mt-3">
            {#if advice.loading}
...
{#snippet sectionHead(no: string, title: string, standfirst: string)}
  <div class="flex items-baseline gap-4">
    <span class="font-display text-2xl font-semibold tabular-nums text-[hsl(var(--brand-gold))]">{no}</span>
    <div>
      <h2 class="font-display text-[clamp(1.6rem,2.6vw,2.4rem)] font-semibold leading-none tracking-tight text-[hsl(var(--brand-navy))]">{title}</h2>
      <p class="mt-2 text-sm text-[hsl(var(--muted-foreground))]">{standfirst}</p>
    </div>
  </div>
{/snippet}
