...
- `502092f` — **item 2: planner live for compound baskets**. DONE, live-verified. "paid 500 rent and got 2000 salary and lent ravi 300" → all 3 legs land with per-line undo tokens; comma-formatted compounds ground; corrections not mis-read; single expenses unaffected; no cross-leg currency bleed. Brutal-review deploy-gate fixes folded in (per-leg span currency guard, non-throwing post-commit + attempted double-write guard, recentContext skip, 8s planner timeout, comma grounding). **Known P2**: a foreign leg in a compound ("$100 hotel") may default to INR if the planner doesn't tag the currency — fails safe (reversible), not the 80× bug. Revisit when the planner prompt is hardened or via per-leg currency extraction.
- `6525fb8` — **item 3: epistemic gate**. DONE, live-verified. Negation/conditional/future/hypothetical money phrasings no longer mint phantom transactions — they route to the copilot (which offers next moves: log-now/wait, set-goal, advice). Zero false-blocks on real logs (verified: "bought a mat for 500", "didn't spend much just 200 chai", "spent 5000 considering diwali" all log; corrections bypass). Brutal-review false-block fixes folded in (strong-verb override, Unicode-boundary native cues, dropped mat/nai, trailing \b, native future cues, per-leg planner epistemic guard). **Known P1 (deferred)**: the single-path parser is not clause-aware, so a mixed "spent 500 but not the 200 loan" could still let the parser pick the wrong leg — rare, and the planner path IS clause-guarded.
- `f29f393` — **item 4: FX resolution worker** (P0-5). DONE, live-verified against prod DB+provider (OMR 5 booked as ₹5 → self-healed to ₹1,240.18, flag cleared). Periodic worker re-converts outage-booked rows, recomputes budgets, invalidates forecast. Brutal-review deploy-gate fixes folded in: CAS update guards on amount+currency+deleted_at (kills the stale-amount corruption race), PATCH re-establishes FX on flagged rows in all branches, poison-pair dead-letter via metadata.fxAttempts after 12 tries, FIFO ordering, space-scoped smoke. **Known P2 (deferred)**: the capture reply shows the 1:1 placeholder ("₹5") during the ≤5-min outage window before the worker heals it — transient, self-correcting; surface a "converting…" note later.
- (next) **item 5** — cardinal sin, for real (P0-6): langDetect fails toward the user (per-user dominant-language prior) not English; native-script STT so register/script survive. The screenshot bug.

### item 5 progress
- `a165641` + `8e97f5f` — **item 5a: langDetect fails toward the user (TEXT)**. DONE, live-verified both directions. Morphological recall catches Manglish the lexicon missed ("ATM il ninnu", "bill adachu" → ml); a decaying 12-turn per-user prior (persisted via the pending blob) makes slips-past-detection prose reply in the user's language ("athinu sesham..." → ml across turns); reverse-cardinal-sin guards verified ("uber 250"/"zomato 450"/"what is my balance" → English from an ml-leaning user). Brutal-review false-positive fixes folded in (dropped -enge/-ille/-nen/standalone mein-wala, ml/ta -nnu/-pannu guard, morphology detects-but-doesn't-tally, fail-toward needs ≥3 prose tokens).
- **item 5b (DONE — `ea68123`,`8527f42`)** — native-script transcription + script/Marathi-aware language resolution. Real-audio smoke (ml + code-mix clips through the simulator) was the gate: ALL voice cases now reply in Malayalam (voice cardinal sin dead). Verified that native saarika output regresses the compound parser (3-expense note → 1 item) AND that saaras:v3 returns native script for ml anyway, so kept TRANSLATE primary (best extraction) + native fallback; kept the languageFromTranscript improvement. **Tracked follow-up (pre-existing, not a 5b regression)**: the parser splits a romanized compound into N expenses but a NATIVE-SCRIPT compound into 1 — needs native-script compound splitting in the batch parser/planner (add native clause joiners + verify batch LLM on native).
- **follow-up flaw found in 5a smoke**: "ATM il ninnu 2000 eduthu" parses as a transfer and surfaces an unlocalized English string "Transfer captures must use the dedicated transfer flow" — a Malayalam-in/English-out leak in the API transfer path (persist.ts). Localize + handle cash-withdrawal phrasing.
