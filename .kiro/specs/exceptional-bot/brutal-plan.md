...
- **item 5b (next)** — native-script transcription: `transcribe.ts` uses Sarvam `/speech-to-text-translate` (returns English), destroying register/script before mirroring. Switch Indic voice to `/speech-to-text` (native script) so voice replies stay in-language. Needs real-audio smoke.
